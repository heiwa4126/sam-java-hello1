...
1. コード書く
2. テストコード書く
3. `gradle test --tests` で部分実行
4. sam build して sam local invoke などでテスト
5. くりかえす
...
```bash
java-hello1$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
java-hello1$ sam local start-api
java-hello1$ curl http://localhost:3000/
```
...
```bash
java-hello1$ sam logs -n HelloWorldFunction --stack-name java-hello1 --tail
```
...
```bash
java-hello1$ cd HelloWorldFunction
...
```bash
aws cloudformation delete-stack --stack-name java-hello1
```
